...
def embed_and_train(doc_list, dataset, data_type, dim, sample):
    ...
    model_parameters = {
        "model_name": "experiment4_" + dataset + "_" + str(dim) + "_word2vec_svm",
...
experiment_folder = os.path.join(LOG_DIRECTORY, "experiment4")
...
for i in range(len(DATA_LIST)):
    ...
    for dim in tqdm(DIM_LIST):
        ...
        test_metrics_file_name = os.path.join(experiment_folder, "experiment4_" + DATASET_NAMES[i] + "_" + str(dim) + "_word2vec_svm.csv")
        metrics_statistics_file_name = os.path.join(experiment_folder, "experiment4_" + DATASET_NAMES[i] + "_" + str(dim) + "_word2vec_svm_stats.csv")
