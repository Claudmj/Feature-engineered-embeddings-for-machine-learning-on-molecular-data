...
def cv_and_train(doc_list, dataset, data_type, sample):
    ...
    model_parameters = {
        "model_name": "experiment7_" + dataset + "_cv_svm",
...
def tfidf_and_train(doc_list, dataset, data_type, sample):
    ...
    model_parameters = {
        "model_name": "experiment7_" + dataset + "_tfidf_svm",
        "dataset_name": dataset
    }
...
def word2vec_and_train(doc_list, dataset, data_type, sample):
    ...
    model_parameters = {
        "model_name": "experiment7_" + dataset + "_word2vec_svm",
        "dataset_name": dataset
    }
...
experiment_folder = os.path.join(LOG_DIRECTORY, "experiment7")
...
for i in range(len(DATA_LIST)):
    ...
    test_metrics_file_name = os.path.join(experiment_folder, "experiment7_" + DATASET_NAMES[i] + "_cv_svm.csv")
    metrics_statistics_file_name = os.path.join(experiment_folder, "experiment7_" + DATASET_NAMES[i] + "_cv_svm_stats.csv")
    ...
    test_metrics_file_name = os.path.join(experiment_folder, "experiment7_" + DATASET_NAMES[i] + "_tfidf_svm.csv")
    metrics_statistics_file_name = os.path.join(experiment_folder, "experiment7_" + DATASET_NAMES[i] + "_tfidf_svm_stats.csv")
    metrics_df.iloc[:NUM_SAMPLES] = metrics_list
    metrics_df.to_csv(test_metrics_file_name)
    metrics_df = metrics_df.apply(pd.to_numeric)
    statistics_df = metrics_df.describe(include='all')
    ...
    if DATA_TYPES[i] != "FASTA":
        ...
        test_metrics_file_name = os.path.join(experiment_folder, "experiment7_" + DATASET_NAMES[i] + "_word2vec_svm.csv")
        metrics_statistics_file_name = os.path.join(experiment_folder,
                                                    "experiment7_" + DATASET_NAMES[i] + "_word2vec_svm_stats.csv")
        metrics_df.iloc[:NUM_SAMPLES] = metrics_list
        metrics_df.to_csv(test_metrics_file_name)
        metrics_df = metrics_df.apply(pd.to_numeric)
        statistics_df = metrics_df.describe(include='all')
